...
export const getTotalLaptopPerDay = async () => {
  try {
    const [result] = await db.query(
      'SELECT pickup_date, COUNT(laptop_id) AS no_of_borrows FROM borrow_records GROUP BY pickup_date ORDER BY pickup_date'
    )
...
export const getAverageLaptopPerMonth = async () => {
  try {
    const [result] = await db.query(
      `SELECT borrows_per_month.month, borrows_per_month.no_of_borrows / DAYOFMONTH(LAST_DAY(CONCAT(borrows_per_month.month, '-01'))) AS avg_daily_borrows FROM (SELECT DATE_FORMAT(pickup_date, '%Y-%m') AS month, COUNT(laptop_id) AS no_of_borrows FROM borrow_records GROUP BY month ORDER BY month) borrows_per_month ORDER BY borrows_per_month.month`
    )
