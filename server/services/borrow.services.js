...
export const getBorrowedLaptopByID = async (borrow_id) => {
  try {
    const [laptop] = await db.query(
      "SELECT br.borrow_id, c.customer_id, CONCAT(c.first_name, ' ', c.last_name) AS customer_name, c.type AS customer_type, col.college AS college_name, l.laptop_id, l.product_name, l.product_os, l.battery_health, s.staff_id, CONCAT(s.first_name, ' ', s.last_name) AS assigned_staff, br.pickup_date, br.return_date FROM borrow_records br JOIN customers c ON br.customer_id = c.customer_id JOIN colleges col ON c.college_id = col.college_id JOIN laptops l ON br.laptop_id = l.laptop_id JOIN staff s ON br.staff_id = s.staff_id WHERE br.borrow_id = ? ORDER BY br.pickup_date DESC",
      [borrow_id]
    )

    return laptop[0]
  } catch (error) {
    console.error('Error fetchingl status: ', error)
    throw error
  }
}
...
export const getBorrowedLaptopByCustomerID = async (customer_id) => {
  try {
    const [laptop] = await db.query(
      "SELECT br.borrow_id, c.customer_id, CONCAT(c.first_name, ' ', c.last_name) AS customer_name, c.type AS customer_type, col.college AS college_name, l.laptop_id, l.product_name, l.product_os, l.battery_health, s.staff_id, CONCAT(s.first_name, ' ', s.last_name) AS assigned_staff, br.pickup_date, br.return_date, p.penalty_id FROM borrow_records br JOIN customers c ON br.customer_id = c.customer_id JOIN colleges col ON c.college_id = col.college_id JOIN laptops l ON br.laptop_id = l.laptop_id JOIN staff s ON br.staff_id = s.staff_id LEFT JOIN penalties p ON br.borrow_id = p.borrow_id WHERE c.customer_id = ? ORDER BY br.pickup_date DESC",
      [customer_id]
    )
